...
DIR2SAVE <- paste0(NICHENET_DIR, "nichenet_microenv",microenv2consider,"/")
dir.create(DIR2SAVE)
...
sign2consider = "NFKB_regulon_combined" # "AP1", "ipEMT" "AP1_regulon" "pEMT" "NFKB_regulon" "NFKB_regulon_combined"
...
# Therefore, users can decide to continue the analysis with a different number of ligands. 
# We recommend to check the selected cutoff by looking at the distribution of the 
# ligand activity values. 
# Here, we show the ligand activity histogram (the score for the 30th ligand is 
...
pdf(paste0(DIR2SAVE,sign2consider,receiver, "_hist_ligand_activity.pdf"), 
    height=4.5, width=10)
print(p_hist_lig_activity)
dev.off()

# write list of ligands to file
write.table(ligand_activities, file=paste0(DIR2SAVE,sign2consider,receiver, "_all_ligands.csv"), 
...
pdf(paste0(DIR2SAVE,sign2consider,receiver,"_nligands", nligands,"_dotplot_ligand_target_network.pdf"), height=7, width=10)
print(p_ligand_target_network)
dev.off()

if (sign2consider=="AP1_regulon" | sign2consider=="NFKB_regulon" | sign2consider=="NFKB_regulon_combined") {
  pdf(paste0(DIR2SAVE,sign2consider,receiver,"_nligands", nligands,"_dotplot_ligand_target_network.pdf"), height=7, width=30)
  print(p_ligand_target_network)
  dev.off()
}
...
p_ligand_aupr
pdf(paste0(DIR2SAVE,sign2consider,receiver, "_nligands", nligands,"_aupr_ligand.pdf"), height=7, width=4.5)
print(p_ligand_aupr)
dev.off()

# Expression of target genes in ipEMT population
# subset seurat object https://satijalab.org/seurat/articles/essential_commands.html
seo <- ScaleData(object = seo)
receiver_seuratobj = subset(x = seo, idents = receiver)
receiver_counts = GetAssayData(object = receiver_seuratobj, slot= "counts")
...
#receiver_seuratobj <- NormalizeData(receiver_seuratobj, verbose = TRUE)
#receiver_seuratobj <- ScaleData(object = receiver_seuratobj)
# how to get raw counts and normalise and get gene exp?
#mapal <- colorRampPalette(RColorBrewer::brewer.pal(11,"RdYlBu"))(256)


heatmap_tmp <- DoHeatmap(object = receiver_seuratobj, 
                         features = order_targets, 
                         group.by = "Patient",
                         group.bar = TRUE,
                         disp.max = 1, 
                         disp.min = 0,
                         raster = FALSE # otherwise blurry when saving plot
                         ) #+ scale_fill_gradientn(colours = rev(mapal))

pdf(paste0(DIR2SAVE,sign2consider,receiver, "_nligands", nligands,"_heatmap_target_tmp.pdf"), height=7, width=12)
print(heatmap_tmp)
dev.off()

# trying complex heatmap 
